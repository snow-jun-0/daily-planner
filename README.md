...
- **구글 연동 4종** — 백엔드 없이 브라우저에서 직접 Google API를 호출합니다.
  - **캘린더 일정 (양방향)** — 시간표 블록을 구글 캘린더 이벤트로 내보내고, 구글의 일정(공휴일·구독 캘린더 포함)을 시간표 위에 함께 표시. 앱 일정·반복 일정·구글 일정의 겹침을 하나의 레이아웃 계산으로 통합 처리하며, 시간표에서 구글 일정을 바로 삭제할 수도 있습니다.
  - **할 일(Tasks) (양방향)** — 앱에서 추가·완료·삭제·날짜(`due`) 변경한 내용을 구글 할 일에 반영하고, 구글에서 만들거나 바꾼 할 일(완료·삭제·날짜 이동 포함)을 로그인 시 앱으로 역동기화. `googleTaskId` + `notes` 마커의 2단 매칭으로 링크가 끊겨도 중복 생성을 막습니다.
  - **D-Day → 구글 캘린더 종일 이벤트** — 다른 기기에서 만든 D-Day도 자동으로 가져옵니다(역동기화).
...
└── src/
    ...
    │                          · Supabase 동기화 · 테마 팔레트(P)
    ├── gcal.ts                구글 캘린더 연동 (GIS 토큰, 다중 캘린더 병합, D-Day 종일 이벤트, 반복 일정 RRULE)
    ...
    └── views/
        ├── DayView.tsx        가로 시간표 격자 + 할 일 + 메모 + 오늘의 습관 (자동 저장, 구글/반복 일정 타이머·삭제)
